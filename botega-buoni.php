...
// ── i18n ─────────────────────────────────────────────────────────────────────
// Priorità 5: carica il textdomain prima di qualsiasi __() nel plugin.
...
add_action( 'plugins_loaded', function () {
    // Ricrea le tabelle se mancanti (es. plugin caricato via FTP senza
    // passare per l'hook di attivazione di WordPress).
    // Nota: nessuna chiamata a __() qui, il textdomain non è ancora caricato.
    if ( get_option( 'bb_db_version' ) !== BB_VERSION ) {
        BB_Database::install();
        BB_Scheduler::register_cron();
    }
...
// ── Ruoli: spostato a init (priorità 10) per evitare __() troppo presto ──────
// Il textdomain viene caricato a init/5, quindi a init/10 __() funziona già.
add_action( 'init', function () {
    BB_Roles::setup();
}, 10 );
