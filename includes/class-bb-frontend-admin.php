...
class BB_Frontend_Admin {
    ...
    public static function init(): void {
        ...
        add_shortcode( 'bb_admin_ruoli',  [ __CLASS__, 'shortcode_ruoli' ] );
    ...
    public static function enqueue_assets(): void {
        ...
        // Shortcode gestione ruoli: carica admin.css (solo admin)
        if ( has_shortcode( $post->post_content, 'bb_admin_ruoli' ) && current_user_can( 'manage_options' ) ) {
            wp_enqueue_style(
                'bb-admin',
                BB_PLUGIN_URL . 'assets/css/admin.css',
                [],
                BB_VERSION
            );
        }
        ...
        wp_localize_script( 'bb-frontend-admin', 'BB_FA', [
            ...
            'i18n'    => [
                'conf_pagato'      => __( 'Contrassegnare come pagato? Verranno inviati i buoni ai destinatari.', 'botega-buoni' ),
                'conf_annulla'     => __( 'Annullare questo ordine?', 'botega-buoni' ),
                'conf_delete'      => __( 'Eliminare definitivamente questo ordine?', 'botega-buoni' ),
                'conf_invia_buoni' => __( 'Inviare i buoni a tutti i destinatari?', 'botega-buoni' ),
                'conf_richiamo'    => __( 'Inviare il richiamo di pagamento all\'acquirente?', 'botega-buoni' ),
                'loading'          => __( 'Elaborazione…', 'botega-buoni' ),
                'ok_richiamo'      => __( 'Richiamo inviato.', 'botega-buoni' ),
                'ok_pdf'           => __( 'PDF generato.', 'botega-buoni' ),
                'inviati'          => __( 'Inviati: %s – Errori: %e', 'botega-buoni' ),
                'err_generic'      => __( 'Si è verificato un errore.', 'botega-buoni' ),
            ],
        ] );
    }
    ...
    // ── Shortcode [bb_admin_ruoli] ────────────────────────────────────────────
    public static function shortcode_ruoli( array $atts = [] ): string {
        if ( ! current_user_can( 'manage_options' ) ) {
            if ( ! is_user_logged_in() ) {
                return '<p class="bb-fa-warn">' . sprintf(
                    wp_kses(
                        __( '<a href="%s">Accedi</a> per gestire i ruoli del plugin.', 'botega-buoni' ),
                        [ 'a' => [ 'href' => [] ] ]
                    ),
                    esc_url( wp_login_url( get_permalink() ) )
                ) . '</p>';
            }
            return '<p class="bb-fa-warn">' . esc_html__( 'Accesso riservato agli Amministratori.', 'botega-buoni' ) . '</p>';
        }
    ...
    // ── AJAX: slot orari per data ─────────────────────────────────────────────
    public static function ajax_get_slots(): void {
        self::check_ajax();
        $date  = sanitize_text_field( $_POST['date'] ?? '' );
        $slots = BB_Public::get_time_slots( $date );
