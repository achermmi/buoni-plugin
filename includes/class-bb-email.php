...
class BB_Email {
    ...
    public static function invia_buono( object $ordine, object $buono ): bool {
        // Se non c'è email destinatario, usa quella dell'acquirente come fallback
        $dest_email = ! empty( $buono->destinatario_email )
            ? $buono->destinatario_email
            : $ordine->acquirente_email;
        if ( empty( $dest_email ) ) return false;
        ...
        $ok = self::send( $dest_email, $subject, $body, $attachments );
    ...
    private static function parse( string $tpl, object $ordine ): string {
        ...
        $result = str_replace( array_keys( $map ), array_values( $map ), $tpl );

        // Espande le variabili buono con tutti i buoni dell'ordine
        $buoni  = BB_Database::get_buoni( (int) $ordine->id );
        $result = self::expand_buoni_vars( $result, $buoni );

        // Rimuove eventuali placeholder {{...}} rimasti non sostituiti
        return preg_replace( '/\{\{[^}]+\}\}/', '', $result );
    }

    /**
     * Sostituisce le variabili buono nel template.
     * {{buoni_dettaglio}} → blocco formattato con tutti i buoni (consigliato).
     * Le variabili singole ({{dest_nome}} ecc.) restano disponibili per buono singolo
     * o per l'email al destinatario.
     */
    private static function expand_buoni_vars( string $tpl, array $buoni ): string {
        if ( empty( $buoni ) ) return $tpl;

        // ── {{buoni_dettaglio}}: blocco leggibile con tutti i buoni ───────────
        $righe = [];
        foreach ( $buoni as $b ) {
            $n      = (int) $b->numero;
            $prefix = count( $buoni ) > 1 ? "Buono #{$n}\n" : '';
            $riga   = $prefix
                . 'Destinatario: ' . esc_html( $b->destinatario_nome ) . ' <' . esc_html( $b->destinatario_email ) . ">\n"
                . 'Codice: '       . esc_html( $b->codice_buono ) . "\n"
                . 'Consegna: '     . ( $b->data_consegna ? date_i18n( 'd.m.Y', strtotime( $b->data_consegna ) ) : '–' );
            if ( ! empty( $b->messaggio ) ) {
                $riga .= "\nMessaggio: " . esc_html( $b->messaggio );
            }
            $righe[] = $riga;
        }
        $dettaglio = implode( "\n\n", $righe );
        $tpl = str_replace( '{{buoni_dettaglio}}', $dettaglio, $tpl );

        // ── Variabili singole (per buono singolo / email destinatario) ────────
        $b   = $buoni[0];
        $map = [
            '{{dest_nome}}'     => esc_html( $b->destinatario_nome ),
            '{{dest_email}}'    => esc_html( $b->destinatario_email ),
            '{{codice}}'        => esc_html( $b->codice_buono ),
            '{{messaggio}}'     => nl2br( esc_html( $b->messaggio ?? '' ) ),
            '{{data_consegna}}' => $b->data_consegna
                ? date_i18n( 'd.m.Y', strtotime( $b->data_consegna ) )
                : '',
        ];
