...
class BB_Database {
    ...
    public static function genera_ref(): string {
        ...
        $ym     = date( 'Ym' );
        $prefix = 'BO-' . $ym . '-';
        // Usa MAX sul suffisso numerico per evitare duplicati in caso di
        // eliminazioni o richieste concorrenti.
        $last = (int) $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COALESCE(MAX(CAST(SUBSTRING(ordine_ref, %d) AS UNSIGNED)), 0)
                   FROM " . self::table_ordini() . "
                  WHERE ordine_ref LIKE %s",
                strlen( $prefix ) + 1,
                $prefix . '%'
            )
        );
    ...
    public static function insert_ordine( array $data ): int|false {
        ...
        if ( ! $ok ) {
            error_log( '[BB] insert_ordine FAILED — ref: ' . $data['ordine_ref'] . ' — ' . $wpdb->last_error );
        }
